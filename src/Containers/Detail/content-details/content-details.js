...
const ContentDetails = (props) => {
  const aboutData = useFetch(`/movie/${props.contentId}?api_key=dce6a338a810ffe30be7528d9a32bf13&language=en-US`);
  const externalIdData = useFetch(`/movie/${props.contentId}/external_ids?api_key=dce6a338a810ffe30be7528d9a32bf13`);

  let render;
  if (aboutData.loading || externalIdData.loading) {
    render = <Spinner />
  } else if (aboutData.error || externalIdData.error) {
    render = <p>Error fetching data, please refresh the website!</p>
  } else {
    render = (
      <div className='container'>
        <div className={classes.TopDetails}>
          <div className={classes.TopDetailLeft}>
            ...
            <p className={classes.Date}>{aboutData.data.release_date ? aboutData.data.release_date.split('-').reverse().join('/') : null}</p>
          ...
          <div className={classes.TopDetailRight}>
            <h2 className={classes.Title} >{aboutData.data.title}</h2>
            ...
            <p className='margin-top'>Budget: {aboutData.data.budget ? `$${aboutData.data.budget.toLocaleString()}` : '/'}</p>
